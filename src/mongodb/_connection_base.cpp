...
namespace flame {
namespace mongodb {
	void _connection_base::execute(std::shared_ptr<coroutine> co, const php::object& ref, std::shared_ptr<bson_t> cmd) {
		exec([cmd] (std::shared_ptr<mongoc_client_t> c, std::shared_ptr<bson_error_t> error) -> std::shared_ptr<bson_t> {
			mongoc_client_t* cli = c.get();
			const mongoc_uri_t *uri = mongoc_client_get_uri(cli);

			bson_t*     reply = bson_new();
			bson_error_t* err = new bson_error_t();
			if(!mongoc_client_command_with_opts(cli, mongoc_uri_get_database(uri), cmd.get(), mongoc_uri_get_read_prefs_t(uri), nullptr, reply, err)) {
