...
namespace udp {
	...
	void declare(php::extension_entry& ext) {
		controller_->on_init([] (const php::array& opts) {
			resolver_ = new udp::resolver(context);
	...
	php::value connect(php::parameters& params) {
		...
		// DNS 地址解析
		resolver_.async_resolve(boost::string_view(s, p - s), boost::string_view(p + 1, e - p - 1), [str, s, s_, co] (const boost::system::error_code& error, udp::resolver::results_type edps) {
			if(error) return co->fail(error);
			// 连接
			boost::asio::async_connect(s_->socket_, edps, [s, s_, co] (const boost::system::error_code& error, const udp::endpoint& edp) {
				if(error) return co->fail(error);
