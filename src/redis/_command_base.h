...
namespace redis {
	...
	public:
		...
		enum reply_type {
			REPLY_SIMPLE,
			REPLY_ASSOC_ARRAY_1, // 返回数据多项按 KEY VAL 生成关联苏族
			REPLY_ASSOC_ARRAY_2, // 第一层普通数组, 第二层关联数组 (HSCAN/ZSCAN)
			REPLY_COMBINE_1, // 与参数结合生成关联数组
			REPLY_COMBINE_2, // 同上, 但偏移错位 1 个参数
			REPLY_PIPE, // 与事务形式相同
			REPLY_EXEC, // 事务特殊处理方式（需要结合上面几种方式）
		};
