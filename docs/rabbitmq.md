...
// 消费
$consumer = $client->consume("queue_name_1", function($consumer, $message) {
...
#### `flame\rabbitmq\consumer client::consume(string $queue_name, callable $cb[, array $options])`
...
#### `yield consumer::run() -> void`
启动消费过程并持续消费 (调用消费回调协程, 阻塞当前协程, 可使用 `close()` 停止消费);
